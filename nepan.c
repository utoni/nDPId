...
#define NEPAN_RECYCLE_EVERY 50000u // Restart worker every N packets (cleanup Wireshark Dissector/Conversations memory impossible atm)
#endif
...
#define NEPAN_MAX_STRING_LENGTH (1u << 20) // Max length of a serialized string as part of a response packet
...
static __attribute__((noreturn)) int nepan_worker_main(int fd)
{
    ...
    uint8_t * pkt = malloc(NEPAN_MAX_PKT_SIZE);
    if (pkt == NULL)
    {
        _exit(3);
    }
...
static int spawn_worker(struct nepan_ctx * const ctx)
{
    int sv[2];
    pid_t pid;
    ...
    ctx->epan_worker.ipc_fd = sv[0];
    ctx->epan_worker.pid = pid;
    return 0;
}

static void reap_worker(struct nepan_ctx * const ctx)
{
    if (ctx->epan_worker.pid > 0)
    {
        kill(ctx->epan_worker.pid, SIGKILL);
        waitpid(ctx->epan_worker.pid, NULL, 0);
        ctx->epan_worker.pid = -1;
    }
    if (ctx->epan_worker.ipc_fd >= 0)
    {
        close(ctx->epan_worker.ipc_fd);
        ctx->epan_worker.ipc_fd = -1;
    }
}

static int respawn_worker(struct nepan_ctx * const ctx)
{
    reap_worker(ctx);
    return spawn_worker(ctx);
}
...
static int parse_response(struct nepan_ctx * const ctx,
                          ndpi_serializer * const serializer)
{
    int fd = ctx->epan_worker.ipc_fd;
    char * kbuf = &ctx->keybuf[0];
    size_t kcap = NEPAN_KEY_MAX;
    char * vbuf = &ctx->valbuf[0];
    size_t vcap = NEPAN_VAL_MAX;
                   ...
                   uint8_t const * const packet)
{
    ...
    if (ctx->epan_worker.ipc_fd < 0 && respawn_worker(ctx) != 0)
    {
        return;
    }

    if (write_full(ctx->epan_worker.ipc_fd, &h, sizeof(h)) <= 0 || write_full(ctx->epan_worker.ipc_fd, packet, h.caplen) <= 0)
    {
        respawn_worker(ctx);
        return;
    }

    if (parse_response(ctx, serializer) != 0)
    {
        respawn_worker(ctx);
    }
}
